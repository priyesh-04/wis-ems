...
class TimeSheetService {
    ...
    async deleteSingleTaskDetails(req, res, next) {
        try {
            ...
            let taskId = existTimesheet.task_details.filter((el) => el != taskDetailsId);
            await TaskDetails.findByIdAndDelete({ _id: taskDetailsId }, (err, done) => {
                if (err) {
                    return res
                        .status(400)
                        .json({ msgErr: true, message: "Error " + err });
    ...
    async timesheetEditReq(req, res, next) {
        try {
            ...
            let edit_reason = [];
            if (existTimesheet) {
                edit_reason = [...existTimesheet?.edit_reason, { name: payload.reason }];
            }
