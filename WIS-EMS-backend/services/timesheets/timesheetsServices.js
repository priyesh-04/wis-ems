...
class TimeSheetService {
  ...
  async taskdetailsByUserDateWise(req, res, next) {
    try {
      ...
      const returnAnswer = (details, totalWorkingDays) => {
        if (!limit || !page) {
          limit = 10;
          page = 1;
        }
      ...
      // for counting total working days
      const getTotalWorkingDaysquery = {
        created_by: userid,
        date: { $gte: start_date, $lte: end_date },
        status: "Present",
      };

      totalWorkingDays = await Timesheets.countDocuments(
        getTotalWorkingDaysquery
      );
        ...
        await Timesheets.aggregate([
          ...
          .exec((err, details) => {
            if (err) {
              ...
              returnAnswer(details, totalWorkingDays);
            }
          });
      } else {
        Timesheets.find({
  ...
  async timesheetEditable(req, res, next) {
    try {
      ...
      const createdByUser = await User.findById(timeSheet.created_by);
      ...
      if (edit_status === "Accepted") {
        const emailData = {
          template: EmailConfig.TEMPLATES.ACCEPT_EDIT_REQ,
          subject: EmailConfig.SUBJECT.ACCEPT_EDIT_REQ,
          email: createdByUser.email_id,
          emailBody: {
            employeeName: createdByUser.name,
            adminName: "Admin",
            reason: payload.reason,
            requestDate: new Date(timeSheet.date).toDateString(),
          },
        };
