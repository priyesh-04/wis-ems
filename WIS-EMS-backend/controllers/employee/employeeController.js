...
const employeeController = {
  async createEmployee(req, res, next) {
    try {
      ...
      if (
        !(
          currentEmployee.user_type === 'Admin' ||
          (currentEmployee.user_type === 'HR' && user_type !== 'Admin')
        )
      ) {
        return next(
          CustomErrorhandler.unauthorization(
            user_type + ' type user not allowed to create!'
          )
        );
      }
      ...
      try {
        ...
        if (exist) {
          return next(
            CustomErrorhandler.alreadyExist('This employee ID already exist!')
          );
        }
        exist = await Employee.exists({ phone_num: req.body.phone_num });
        if (exist) {
          return next(
            CustomErrorhandler.alreadyExist('This phone number already exist!')
          );
        }
        exist = await Employee.exists({ email_id: req.body.email_id });
        if (exist) {
          return next(
            CustomErrorhandler.alreadyExist('This email ID already exist!')
          );
        }
      } catch (err) {
        return next(err);
